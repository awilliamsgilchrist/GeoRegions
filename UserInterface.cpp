    ...
    if (m_subRegionType!=Region::RegionType::UnknownRegionType) {
        ...
        if (data != "") {
            ...
            if (region != nullptr) {
                ...
                //Done
                m_currentRegion->appendChild(region);
                //ENDOF TODO
...
void UserInterface::edit()
{
    ...
    if (input!="")
    {
        ...
        if (valid && id>0)
        {
            ...
            //Done
            region = m_currentRegion->findID(id);
            //ENDOF TODO
            if (region!=nullptr)
            {
                std::cout << "Editing: ";
...
void UserInterface::remove()
{
    ...
    if (input!="")
    {
        bool valid;
        unsigned int id = convertStringToUnsignedInt(input, &valid);
        //See the next section of TODO
        valid = static_cast<bool>(m_currentRegion->findID(id));
        if (valid && id>0)
        {
            // TODO: Look up the region by Id and assign it to the region variable
            //Done
            Region* parentRegion = m_currentRegion->findParentByChildID(id);
            Region* childRegion = parentRegion->findID(id);
